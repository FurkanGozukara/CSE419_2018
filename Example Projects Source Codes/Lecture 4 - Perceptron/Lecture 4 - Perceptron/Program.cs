...
namespace Lecture_4___Perceptron
{
    class Program
    {
            ...
            do
            {
                ...
                for (int p = 0; p < patternCount; p++)
                {
                    ...
                    if (localError != 0)
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            Console.WriteLine("weight i({0}) change rate is {1} meanwhile local error is {2} and input p i is {3}",
                                i, (learningRate * localError * inputs[p, i]), localError, inputs[p, i]);
                            Console.Write("weight is " + weights[i] + " >  ");
